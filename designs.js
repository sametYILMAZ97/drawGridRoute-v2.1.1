...
function makeGridd(height, width) {
    var table = document.getElementById("pixel-canvas"),rIndex,cIndex;
    ...
    for (let y = 0; y < 10; y++) {
        for (let x = 0; x < 10; x++) {
            table.rows[y].cells[x].onmousemove = function()
            {
                //cell.style.backgroundColor = color.value;
                    ...
                    if(i == 1){
                        var xPosition = rIndex.toString(10);
                        var yPosition = cIndex.toString(10);
                        var position = xPosition + yPosition;
                        
                        //var position = rIndex + cIndex;

                        //console.log("position:" + position);
                        //console.log("x:" + rIndex + "," + "y:" + cIndex);
                        dirtypos.push(position);
                        //listede tutma
                        /* let liste = [];
                        liste.push(rIndex , cIndex);
                        console.log(liste); */
                    }
    ...
    console.log(dirtypos);
    
    //temizlik
    //var cleanpos =[...new Set([...dirtypos])]
    const myTimeout = setTimeout(cleaning, 2000);
    function cleaning(){
        var cleanpos = new Set([ ...dirtypos])
        console.log(cleanpos);
        
        let stringCleanPos = Array.from(cleanpos);
        //let array = [...mySet];
        document.getElementById("cleanpos").innerHTML = stringCleanPos;
    }
...
.on('mousedown', 'td', makeGridd)
.on('mousedown', 'td', drag);


/* 
var mqtt=require('mqtt');
var client  = mqtt.connect("mqtt://192.168.1.100:1883:60");
...
//var socket = io("http://192.168.253.105:5000");
